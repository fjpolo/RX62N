...
/*
 * The two tasks created in Test001 are almost identical, the
 * only difference between them being the LED being toggled.
 * This duplication can be removed by, instead, creating two
 * instances of a single task implementation. The task parameter
 * can then be used to pass into each task the LED to be toggled.
 */
