...
/*
 *  This example provides similar functionality to _Interrupt,
 * but without using a semaphore, and without creating a task
 * specifically to perform the processing necessitated by the
 * interrupt. Instead, the processing is performed by the RTOS
 * daemon task.
 *
 *  The interrupt service routine increments a variable called
 * ulParameterValue each time it executes. ulParameterValue is
 * used as the value of ulParameter2 in the call to
 * xTimerPendFunctionCallFromISR(), so will also be used as the
 * value of ulParameter2 in the call to vDeferredHandlingFunction()
 * when vDeferredHandlingFunction() is executed by the daemon task.
 * The function’s other parameter, pvParameter1, is not used in
 * this example.
 *
 */
