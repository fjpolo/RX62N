...
*               : 01.12.2015 1.05   get_random_number add.
*               : 15.04.2016 1.06   Fixed count timing for 10ms tick software
*               :                     counter variable "tcpudp_time_cnt".
*               : 15.04.2016 1.06-a Customized for system that has no ET_LINKSTA pin using
******************************************************************************/
...
H lan_write(UB lan_port_no, B *header , H header_len, B *data , H data_len)
{
    ...
    if (R_ETHER_OK == driver_ret)
    {
        ...
        if (buf_size >= framesize)
        {
            ...
            driver_ret =  R_ETHER_Write_ZC2_SetBuf(0, (uint16_t)framesize);
            if (R_ETHER_OK == driver_ret)
            {
                t4_stat.t4_snd_cnt++;
                t4_stat.t4_snd_byte += framesize;
                return 0;
            }
        }
    }
...
void report_error(UB lan_port_no, H err_code, UB *err_data)
{
    ...
    switch (err_code)
    {
        ...
        case RE_IGMP_HEADER1:
            t4_stat.re_igmp_header1_cnt++;
            break;
        case RE_IGMP_HEADER2:
            t4_stat.re_igmp_header2_cnt++;
            break;
        case RE_DHCP_ILLEGAL:
            t4_stat.re_dhcp_header1_cnt++;
            break;
        case RE_DHCP_SND_TIMEOUT:
            t4_stat.re_dhcp_header2_cnt++;
            break;
        default:
            break;
    }
...
H lan_check_link(UB lan_port_no)
{
    ...
    if(R_ETHER_OK == R_Ether_CheckLink_ZC(lan_port_no))
    {
        res = 1;
    }
    else
