...
/*
 *  This example uses a binary semaphore to unblock a task from
 * an interrupt service routine—effectively synchronizing the
 * task with the interrupt.
 *
 *  A simple periodic task is used to generate a software interrupt
 * every 250 milliseconds. A software interrupt is used for
 * convenience because of the complexity of hooking into a real
 * interrupt in some target environments. Listing 92 shows the
 * implementation of the periodic task. Note that the task prints
 * out a string both before and after the interrupt is generated.
