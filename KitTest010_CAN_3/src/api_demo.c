...
* Version		2.05
* Tool-Chain	RX Standard Toolchain 2.0.7
* Platform		RSK-RX62N (ROK5562N0C001BR), RX/62N Eval-board (ROK5562N0C010BR).
* Description	Demo application program
				...
				OPERATION:
				The demo transmits and receives frames with CAN-ID 1 for standard
                ID mode, or 0x000A0001 for extended or mixed ID mode by 
				default. The default demo CAN-ID value is set in main().
...
*******************************************************************************
* History
* Mar 22 '10	REA		For RX62N with new CAN API on 62N Eval Board.
* Apr 15 '10	REA 	Test mode changes.
* Aug 13 '10	REA		Release for RSK-RX62N (ROK5562N0C001BR). Also runs on 
						RX/62N Eval-board (ROK5562N0C010BR).
* DEC 16 2011   REA     Added support for extended ID frame mode.
...
#define MAX_CHANNELS 2 /* Some parts have moe than one channel. */
...
******************************************************************************/
/* Data */
/* Errors. Peripheral and bus errors. Prepared for two buses, but only one in 
RX62N. */
static uint32_t		error_bus_status[2] = {R_CAN_STATUS_ERROR_ACTIVE, R_CAN_STATUS_ERROR_ACTIVE}; 
static uint32_t     error_bus_status_prev[2] = {R_CAN_STATUS_ERROR_ACTIVE, R_CAN_STATUS_ERROR_ACTIVE}; 
...
void main(void)
{
	...
	DisplayString(LCD_LINE2, "TxRxDemo");
	
	/* Blink LEDs. */
	TestLEDs(20);

    /* Set starting mailbox IDs for the demo*/
	...
	**************************************/
	...
	/* Test modes. With Internal loopback mode you only need 
	one board! */
	...
	/* Send several test messages back to back to demonstrate how to do this as fast as possible. */
	if (FRAME_ID_MODE == STD_ID_MODE )
    {
    ...
    {
	    api_status |= R_CAN_TxSetXid(0, CANBOX_TX, &tx_dataframe, DATA_FRAME);
    }
...
static void CanPollDemo(void)
{
	...
	#if TEST_FIFO
	if (tx_fifo_flag == 1)
    /* Note that this function may be called to fast from the time between 
    SW1 being executed, and the time this if statement is run. This means that the CAN frames 
    have not gone out yet, and so TxCheck will not be OK. */
    {
        tx_fifo_flag = 0;
        api_status = R_CAN_TxCheck(0, TX_FIFO_MBX);
		if (api_status == R_CAN_OK)
		{
            /* $REA FIX - Never gets here. */
            ORG_LED2 = LED_ON;
			DisplayString(LCD_LINE1, "TxFifoOK");
			LcdShow4DigHex(tx_dataframe.data[0], tx_dataframe.data[1], 16);
			LcdShow4DigHex(tx_dataframe.data[2], tx_dataframe.data[3], 20); LCD_FLASH
	    }
        else
            DisplayString(LCD_LINE1, "TxFifoNo");
    }
	#endif
...
static void CanIntDemo(void)
{
	...
	*************************************************************************/
	...
	#if TEST_FIFO
    	if (tx_fifo_flag == 1)
        /* Success detected by FIFO TX interrupt. */
        {
            tx_fifo_flag = 0;
            ORG_LED2 = LED_ON;
    		DisplayString(LCD_LINE1, "TxFifoOK");
	...
	if (can0_rx_remote_frame_flag == 1)
	{
        ...
        {
            R_CAN_TxSetXid(0, CANBOX_REMOTE_TX, &remote_frame, DATA_FRAME);             
        }     
...
{	
    ...
    if (FRAME_ID_MODE == STD_ID_MODE)
    { 
	    api_status |= R_CAN_RxSet(0, CANBOX_RX, g_rx_id_default, DATA_FRAME);
    }
    else
    {
	    api_status |= R_CAN_RxSetXid(0, CANBOX_RX, g_rx_id_default, DATA_FRAME);        
    }
	
	/********	Init. demo Tx dataframe RAM structure	********/	
	/* Standard id. Choose value 0-0x07FF (2047). */
	tx_dataframe.id		=	g_tx_id_default;
	tx_dataframe.dlc		=	8;
	...
	#if TEST_FIFO
		tx_fifo_dataframe.id		=	g_tx_id_default+1;
		tx_fifo_dataframe.dlc		=	4;
		tx_fifo_dataframe.data[0]		=	0x00;
		tx_fifo_dataframe.data[1]		=	0x11;
		tx_fifo_dataframe.data[2]		=	0x22;
		tx_fifo_dataframe.data[3]		=	0x33;
	#endif

	/* Mask for receive box. Write to mask only in Halt mode. */
	/* 0x7FF = no mask. 0x7FD = mask bit 1, for example; If receive ID is set 
	to 1, both ID 1 and 3 should be received. */
    ...
    {
        R_CAN_RxSetXid(0, CANBOX_REMOTE_RX, REMOTE_TEST_ID, REMOTE_FRAME);        
    }
	/***********************************************************************/
	
	/* Set frame buffer id so LCD shows correct receive ID from start. */
	 rx_dataframe.id = g_rx_id_default;
...
static void HandleCanBusState(uint8_t	ch_nr)
{
	...
	{	
		...
		/* Send Error state on both channels. Maybe at least one is up. 
         ...
         if (FRAME_ID_MODE == STD_ID_MODE )
         {
		    R_CAN_TxSet(0, g_tx_id_default, &err_tx_dataframe, DATA_FRAME);
         }
         else
         {
		    R_CAN_TxSetXid(0, g_tx_id_default, &err_tx_dataframe, DATA_FRAME);             
         }
		//R_CAN_TxSet(1, g_tx_id_default, &err_tx_dataframe, DATA_FRAME);
	 }
